...
- **Bug de "Flechas totales" incompleto en el resumen**: `cargarResumen()` sumaba `flechas.size` (filas reales de la tabla `Flecha`), que no existen para series avanzadas solo con el contador (sin Guardar) — esas solo tienen `Serie.flechasEstimadas`. `cargarDetalle()` ya sumaba bien con `flechasEstimadas ?: conteo real` (`numeroFlechas`); se aplicó el mismo criterio en `cargarResumen()`. El mapa de calor (`conteoPorCoordenada`) se queda solo con flechas reales a propósito — las estimadas no tienen coordenada que graficar. Es un fix de cómputo, no de datos guardados: se corrige solo al volver a abrir el resumen, sin migración.
